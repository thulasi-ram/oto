...
[`oto_ingest_process_duration_seconds`](oto_ingest_process_duration_seconds.md).
...
deliveries ([`oto_ingest_duplicate_total`](oto_ingest_duplicate_total.md)) and then as gaps.
...
- `unavailable` slow → you are queueing before shedding; see
  [`oto_ingest_shed_total`](oto_ingest_shed_total.md).
- `undecodable` slow → a huge body being parsed and refused. Check `body_too_large` on
  [`oto_ingest_rejected_total`](oto_ingest_rejected_total.md).

# oto_stream_events_dropped_total
...
moves together with [`oto_stream_resync_total`](oto_stream_resync_total.md).
...
1. The ratio, not the raw count:
   ...
   [`oto_stream_connections`](oto_stream_connections.md).
2. Is it correlated with an ingest spike? Compare with
   [`oto_ingest_alerts_total`](oto_ingest_alerts_total.md). A storm explains it.
3. Is it one pod? Drops concentrated on a single pod with normal connection counts points at that
   pod, not at the clients.
4. [`oto_stream_events_coalesced_total`](oto_stream_events_coalesced_total.md): if coalescing is

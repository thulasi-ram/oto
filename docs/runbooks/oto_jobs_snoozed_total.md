...
| ordering-gate reasons — `awaiting_root`, `awaiting_predecessor` | dispatch | the thread head has not reached this slot yet (see [`oto_thread_order_decisions_total`](oto_thread_order_decisions_total.md)) |
...
- **`awaiting_*` sustained** — deliveries are stacking behind a thread head. Cross-check
  [`oto_thread_head_wait_seconds`](oto_thread_head_wait_seconds.md); if that is also climbing, one
...
- If a thread is genuinely stuck, the gate will recover it and record why — see
  [`oto_thread_gap_recovered_total`](oto_thread_gap_recovered_total.md).

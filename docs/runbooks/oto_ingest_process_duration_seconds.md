# oto_ingest_process_duration_seconds
...
1. [`oto_job_queue_depth`](oto_job_queue_depth.md) for `queue="ingest"` — is it one slow batch or a
   thousand queued ones?
2. [`oto_job_duration_seconds`](oto_job_duration_seconds.md) for `kind="ingest.process_batch"`,

...
minutes and shows up on [`oto_jobs_dead_total`](oto_jobs_dead_total.md).
...
- If it is deploy-shaped and the previous build was clean, roll back — but check
  [`oto_jobs_unknown_version_total`](oto_jobs_unknown_version_total.md) first, because rolling back

...
| `ingest.process_batch` | scales with batch size; see [`oto_ingest_process_duration_seconds`](oto_ingest_process_duration_seconds.md) |
...
Latency here becomes depth on [`oto_job_queue_depth`](oto_job_queue_depth.md) and, for the ingest
queue, 503s on [`oto_ingest_shed_total`](oto_ingest_shed_total.md). Slow jobs are how a healthy
...
3. For `deliver.*`: the provider's own latency, and whether snoozes for `rate_limited` are climbing
   ([`oto_jobs_snoozed_total`](oto_jobs_snoozed_total.md)).

...
(see [`oto_jobs_snoozed_total`](oto_jobs_snoozed_total.md)); the whole point is that a busy thread
...
[`oto_jobs_dead_total`](oto_jobs_dead_total.md), and *that* is unrecoverable work.
...
- `config_invalid` / `auth_expired` — go to [`oto_jobs_dead_total`](oto_jobs_dead_total.md); the

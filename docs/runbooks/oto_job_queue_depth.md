...
| `discarded` / `cancelled` | terminal; correlates with [`oto_jobs_dead_total`](oto_jobs_dead_total.md) |
...
is [`oto_ingest_shed_total`](oto_ingest_shed_total.md) with `reason="queue_depth"`. That number is
...
2. [`oto_job_duration_seconds`](oto_job_duration_seconds.md) for that queue: fewer, slower jobs or
   more, normal ones?

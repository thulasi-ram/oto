# oto_stream_events_fetched_total
...
[`oto_stream_events_published_total`](oto_stream_events_published_total.md) is normally much larger.

## Why there is no response

No value is wrong. Its uses are comparative:

- Against [`oto_stream_notify_received_total`](oto_stream_notify_received_total.md): fetches should
  track doorbells. Far more fetches than doorbells means the reconciling poll is carrying the
  stream — see [`oto_stream_poll_recovered_total`](oto_stream_poll_recovered_total.md).
...
[`oto_stream_fetch_errors_total`](oto_stream_fetch_errors_total.md).

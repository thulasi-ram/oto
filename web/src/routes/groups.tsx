/**
 * `/groups` — Alertmanager's notification grouping, mirrored.
 *
 * An **AlertGroup** is one generation of one Alertmanager notification group,
 * derived from `(source, receiver, groupLabels)`. It owns exactly one chat
 * thread, which is why a new generation is what produces a new root message
 * rather than a reply. Its members are the alerts whose current firing episode
 * was notified under it.
 *
 * ⛔ AN AlertGroup IS NOT A CASE, AND IT IS NOT A CORRELATION. A Case is one
 * firing episode of ONE alert and is what a human acknowledges; a group is
 * *how the notification was batched*, and Alertmanager — not oto — decided it.
 * Calling it a case on screen would claim oto had reasoned about which alerts
 * belong together, which it has not and will not. It is not an incident either:
 * oto records signals, and there is no object here for a human response.
 *
 * ⛔ THIS SCREEN IS NOT IN THE RAIL, ON PURPOSE. Grouping is plumbing an
 * operator arrives at from a Slack card or from a case they were already
 * looking at, never a destination they set out for — see the ⛔ beside `NAV` in
 * `AppShell.tsx`. Every `/groups` link oto has ever minted still resolves here,
 * which is why the screen exists at all.
 *
 * It is also deliberately *not* "the alert list, grouped" — that is
      ...
      <FilterRow class="shrink-0">
        ...
        >
          <TextFieldLabel class="sr-only-focusable">
            Search group titles and grouping labels
          </TextFieldLabel>
          <TextFieldInput
            id="group-q"
            type="search"
            placeholder="Search group titles and grouping labels…"
            onKeyDown={(e) => {
      ...
      <Switch>
        ...
        <Match when={all().length === 0}>
          <PageEmptyState
            ...
            body="A group appears when Alertmanager routes a notification to oto. If nothing has been routed yet, there is nothing to have been grouped."
          />
        </Match>
        <Match when={true}>
          <div class="min-h-0 flex-1 overflow-auto">
            <ul>
              <For each={all()}>{(g) => <GroupRow group={g} />}</For>
            </ul>
      ...
      >
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-meta text-ink-muted">
            ...
            <Show when={unacked() > 0}>
              <span title="Members whose open case carries no receipt yet.">

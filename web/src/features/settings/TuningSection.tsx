    ...
    >
      <div class="flex flex-wrap items-baseline gap-x-sm gap-y-2xs">
        ...
        <Show when={props.route.group_by_all}>
          <span
            class="rounded-chip border border-line-strong px-1 text-micro leading-4 text-ink-muted"
            title="group_by: ['...'] groups by every label, so no alert group ever accumulates a second member: every alert gets its own group and its own chat thread. That fix is in alertmanager.yml, not on this screen."
          >
            group_by: ...
          </span>
        </Show>
  ...
  <Panel>
    ...
    <div class={cn("border-t border-line bg-raised", PANEL_BODY)}>
      <p class="text-meta leading-snug text-ink-subtle">
        ...
        <code class="font-mono text-ink-muted">pod</code>, no alert group ever accumulates a second
        member, so every replica gets its own group and its own chat thread. That fix is in{" "}

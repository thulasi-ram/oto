...
oto is a flight recorder for alerts. Read [CONTEXT.md](CONTEXT.md) first: it carries the domain
language, the module map and the layering rules, and most refused changes are refused for
contradicting one of the three. [docs/design/SPEC.md](docs/design/SPEC.md) is binding and meant to
be implemented literally; it changes only through an ADR in the same commit.

## Ground rules

- **MIT licence** — see [LICENSE](LICENSE) and [ADR 0019](docs/adr/0019-mit-licence.md). No `ee/`
...
documented in [docs/setup/configuration.md](docs/setup/configuration.md).

## The task runner

`just` is the only task runner ([ADR 0021](docs/adr/0021-correctness-and-testing-strategy.md): a
...
[`docs/releasing.md`](docs/releasing.md) is the whole procedure: what the eight checks are for, what
...
[SCOPE-BOUNDARY.md](docs/design/SCOPE-BOUNDARY.md), restated as SPEC §P-18, that bans a fixed list
...
[docs/concepts.md § 6, The boundary](docs/concepts.md#6-the-boundary) — and is not repeated here,

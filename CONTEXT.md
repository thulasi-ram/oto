...
| **Stanza** | One **named, ordered unit of a rendered message**: `title`, `body`, `fields`, `members`, `trail`, `rule`, `actions`, `footer`. The set is SPEC §H.7's block budget, which enumerated the eight and had no collective noun for them. The renderer always has one — configuration may substitute a Stanza's TEXT, and never authors its structure, its order or a ninth name. **Eight names, FOUR wordable** — `title`, `body`, `rule` and `footer`, the ones that are PROSE. The other four are declared and REFUSED, each with its own sentence, because they are structure wearing a stanza's name: `fields` is a grid of up to ten separately-budgeted cells in a binding order that decides what is shed on overflow, so one line would replace the grid rather than re-word it; `members` and `trail` are SEQUENCES, and ADR 0037 already refuses enumeration in its own words ("without `{% for %}` a Wording cannot iterate ... this is a real ceiling and it is accepted deliberately"); `actions` carries button labels bound to stable `action_id`s an interaction handler matches on, so re-wording one is an interaction change wearing a wording's clothes. All eight names stay in the enum so the vocabulary does not fork from §H.7, and a refusal answers with a reason rather than with absence from a menu. It is **not** a `block` — that is Slack Block Kit's own type name, and it cannot name the same unit on the webhook renderer, where a Stanza is one JSON string with no visual identity. |
| **Wording** | One typed, matcher-guarded **Liquid template that produces the text of one Stanza**. It chooses words; it cannot choose structure, order, colour, an audience, a link or a destination. Its `when` clause reuses `Matcher{Name,Op,Value}` and the `Reason` enum verbatim (ADR 0017), so conditionality is two Wordings with different matchers and never a branch inside one — a matcher lets the UI show *which Wording won*, and a buried `{% if %}` cannot. It is **not** a "template": a Wording is one line of prose for one Stanza with no branching, no iteration, no partials, no inheritance and no layout, and calling it a template invites every document-shaped expectation this design refuses. Its output carries **neutral marks, never a provider's punctuation** — a Dialect spells them per provider (ADR 0048). |

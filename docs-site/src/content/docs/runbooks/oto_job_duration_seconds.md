...
| `ingest.process_batch` | scales with batch size; see [`oto_ingest_process_duration_seconds`](/runbooks/oto_ingest_process_duration_seconds/) |
...
Latency here becomes depth on [`oto_job_queue_depth`](/runbooks/oto_job_queue_depth/) and, for the ingest
queue, 503s on [`oto_ingest_shed_total`](/runbooks/oto_ingest_shed_total/). Slow jobs are how a healthy

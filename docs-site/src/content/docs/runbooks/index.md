...
[`alert-search-partial-match.md`](/oto/runbooks/alert-search-partial-match/) is a one-time, operator-run SQL
...
|---|---|---|
| [`oto_ingest_accepted_total`](/oto/runbooks/oto_ingest_accepted_total/) | on **absence** | Batches durably persisted and enqueued |
| [`oto_ingest_alerts_total`](/oto/runbooks/oto_ingest_alerts_total/) | no | Individual alerts accepted |
| [`oto_ingest_duplicate_total`](/oto/runbooks/oto_ingest_duplicate_total/) | no | Batches collapsed by `ingest_dedup`; non-zero is healthy |
| [`oto_ingest_rejected_total`](/oto/runbooks/oto_ingest_rejected_total/) | yes | Observations refused, by reason, with the evidence kept |
| [`oto_ingest_shed_total`](/oto/runbooks/oto_ingest_shed_total/) | yes | Deliberate 503 backpressure, by reason |
| [`oto_ingest_duration_seconds`](/oto/runbooks/oto_ingest_duration_seconds/) | yes | Accept latency; p99 budget 250 ms |
| [`oto_ingest_process_duration_seconds`](/oto/runbooks/oto_ingest_process_duration_seconds/) | yes | `ingest.process_batch` latency |
| [`oto_ingest_fingerprint_mismatch_total`](/oto/runbooks/oto_ingest_fingerprint_mismatch_total/) | yes | Wire fingerprint disagreed with oto's recomputation |
| [`oto_clock_skew_seconds`](/oto/runbooks/oto_clock_skew_seconds/) | yes | `received_at − startsAt`; measured, never corrected |
...
| Metric | Alertable | One line |
|---|---|---|
| [`oto_jobs_enqueued_total`](/oto/runbooks/oto_jobs_enqueued_total/) | no | Jobs inserted, by kind and queue |
| [`oto_jobs_started_total`](/oto/runbooks/oto_jobs_started_total/) | no | Executions begun |
| [`oto_jobs_succeeded_total`](/oto/runbooks/oto_jobs_succeeded_total/) | no | Executions that returned nil |
| [`oto_jobs_failed_total`](/oto/runbooks/oto_jobs_failed_total/) | yes | Executions that returned an error, by §G.6 class |
| [`oto_jobs_dead_total`](/oto/runbooks/oto_jobs_dead_total/) | **yes, page** | Jobs that will never run again |
| [`oto_jobs_panics_total`](/oto/runbooks/oto_jobs_panics_total/) | **yes, page** | Panics recovered in a handler. Always a bug |
| [`oto_jobs_unknown_version_total`](/oto/runbooks/oto_jobs_unknown_version_total/) | **yes, page** | Payload newer than this binary understands |
| [`oto_jobs_snoozed_total`](/oto/runbooks/oto_jobs_snoozed_total/) | yes, sustained | Deferrals that consume no attempt |
| [`oto_job_duration_seconds`](/oto/runbooks/oto_job_duration_seconds/) | yes | Wall time of one execution |
| [`oto_job_queue_depth`](/oto/runbooks/oto_job_queue_depth/) | yes | Backlog by queue and River state |
...
| Metric | Alertable | One line |
|---|---|---|
| [`oto_thread_order_decisions_total`](/oto/runbooks/oto_thread_order_decisions_total/) | no | Gate verdicts, by action and reason |
| [`oto_thread_gap_recovered_total`](/oto/runbooks/oto_thread_gap_recovered_total/) | **yes** | Slots advanced past unsent. Sustained means a channel is broken |
| [`oto_thread_head_wait_seconds`](/oto/runbooks/oto_thread_head_wait_seconds/) | yes | How long a delivery waited behind the head |

## Delivery and inbound interactions

| Metric | Alertable | One line |
|---|---|---|
| [`oto_delivery_claim_lost_total`](/oto/runbooks/oto_delivery_claim_lost_total/) | **yes, page** | A send landed and could not be recorded |
| [`oto_slack_unknown_action_total`](/oto/runbooks/oto_slack_unknown_action_total/) | **yes** | A button a human pressed that oto answered 200 and could not route |
...
| Metric | Alertable | One line |
|---|---|---|
| [`oto_stream_connections`](/oto/runbooks/oto_stream_connections/) | no | Live SSE connections on this pod |
| [`oto_stream_events_published_total`](/oto/runbooks/oto_stream_events_published_total/) | no | Events accepted into a subscriber's buffer |
| [`oto_stream_events_fetched_total`](/oto/runbooks/oto_stream_events_fetched_total/) | no | `ui_events` rows read by the bridge |
| [`oto_stream_events_coalesced_total`](/oto/runbooks/oto_stream_events_coalesced_total/) | no | Frames superseded inside the 250 ms window |
| [`oto_stream_notify_received_total`](/oto/runbooks/oto_stream_notify_received_total/) | no | LISTEN/NOTIFY doorbells received |
| [`oto_stream_events_dropped_total`](/oto/runbooks/oto_stream_events_dropped_total/) | yes | Buffer full; oto never blocks a writer for a reader |
| [`oto_stream_resync_total`](/oto/runbooks/oto_stream_resync_total/) | yes | "Refetch everything", by reason |
| [`oto_stream_notify_malformed_total`](/oto/runbooks/oto_stream_notify_malformed_total/) | yes | NOTIFY payloads that did not parse |
| [`oto_stream_listener_reconnects_total`](/oto/runbooks/oto_stream_listener_reconnects_total/) | yes | LISTEN connection re-established |
| [`oto_stream_poll_recovered_total`](/oto/runbooks/oto_stream_poll_recovered_total/) | yes, on a spike | Events the reconciling poll caught |
| [`oto_stream_fetch_errors_total`](/oto/runbooks/oto_stream_fetch_errors_total/) | yes | Failed catch-up reads of `ui_events` |
...
| `oto_delivery_dead_total` | `notification_deliveries.status = 'dead'` with `error_class`; per-job deaths are on [`oto_jobs_dead_total`](/oto/runbooks/oto_jobs_dead_total/), which is alertable and paged |
...
[`oto_render_invalid_total`](/oto/runbooks/oto_render_invalid_total/). Its entry here was not just thin, it was
...
shipped as [`oto_thread_gap_recovered_total`](/oto/runbooks/oto_thread_gap_recovered_total/). The SPEC and the

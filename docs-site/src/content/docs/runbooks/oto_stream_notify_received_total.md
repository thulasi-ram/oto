...
[`oto_stream_poll_recovered_total`](/runbooks/oto_stream_poll_recovered_total/) (the poll doing more work
than it should) rather than this counter going quiet.

Useful comparisons:

- Flat while [`oto_stream_events_fetched_total`](/runbooks/oto_stream_events_fetched_total/) climbs → the
  LISTEN connection is not delivering; check
  [`oto_stream_listener_reconnects_total`](/runbooks/oto_stream_listener_reconnects_total/).

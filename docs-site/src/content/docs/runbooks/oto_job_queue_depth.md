...
| `discarded` / `cancelled` | terminal; correlates with [`oto_jobs_dead_total`](/runbooks/oto_jobs_dead_total/) |
...
is [`oto_ingest_shed_total`](/runbooks/oto_ingest_shed_total/) with `reason="queue_depth"`. That number is
...
2. [`oto_job_duration_seconds`](/runbooks/oto_job_duration_seconds/) for that queue: fewer, slower jobs or
   more, normal ones?

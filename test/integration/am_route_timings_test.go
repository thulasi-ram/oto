...
func TestEveryMigrationDownTo00028IsReversible(t *testing.T) {
	...
	// already recorded 77 permanently short of the new schema — `goose up` reporting
	// nothing to do while the application 500'd on a column that did not exist. A
	// retired concept is retired by a NEW migration that drops it.
	//
	// 00079 links presentation to routing and records what rendered a card. Two
	...
	//
	// ⚠️ ITS Down WARNS RATHER THAN REFUSING, UNLIKE 00078'S, and the asymmetry is
	...
	// bytes and `notification_templates` still holds every template. 00078 refuses
	...
	for _, col := range []string{"template_id", "template_version"} {
		if n := countColumns("notification_deliveries", col); n != 1 {
			t.Fatalf("notification_deliveries.%s is absent at migration 79 (found %d); "+
				"00079's Up exists so that \"why did my card read like that\" needs one row "+
	...
	if n := countColumns("notification_policies", "template_id"); n != 1 {
		t.Fatalf("notification_policies.template_id is absent at migration 79 (found %d); "+
	...
	for _, col := range []string{"template_id", "template_version"} {
		if n := countColumns("notification_deliveries", col); n != 0 {
			t.Fatalf("notification_deliveries.%s survived 00079's Down (found %d)", col, n)
		}
	}
	if n := countColumns("notification_policies", "template_id"); n != 0 {
		t.Fatalf("notification_policies.template_id survived 00079's Down (found %d)", n)
	}

	// 00078 is a plain CREATE TABLE, so its Down is a DROP guarded by a row count —
	...
	if n := countTables("notification_templates"); n != 1 {
		t.Fatalf("notification_templates is absent at migration 78 (found %d); 00078's Up "+
	...
	if _, err := env.pool.Exec(env.ctx,
		...
		t.Fatal("notification_templates accepted format 'handlebars' at migration 78; " +
	...
	if n := countTables("notification_templates"); n != 0 {
		t.Fatalf("notification_templates still exists after 00078's Down (found %d); the Down "+
			"is a guarded DROP and a Down that runs cleanly and removes nothing is the failure "+
			"this file exists to catch", n)
	}
	// ⭐ AND `wordings` IS BACK. 00078's Down drops its own table and RESTORES the one
	// it replaced, because a Down that removed this migration's table and left the
	// earlier one missing would leave the database at a version 00077's own Down
	// cannot act on — not a rollback, a second broken state.
	if n := countTables("wordings"); n != 1 {
		t.Fatalf("wordings was not restored by 00078's Down (found %d); the release below "+
			"00078 reads that table, and 00077's Down below this line cannot run without it", n)
	}

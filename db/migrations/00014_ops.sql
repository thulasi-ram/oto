...
CREATE TABLE alert_quality_daily (
  ...
  occurrences       INT         NOT NULL DEFAULT 0,  -- vocab:allow -- migration history: this file states the schema as it was at its own version, and a shipped migration is not editable. ADR 0036 renamed the entity and 00052 carries the rename.
  notifications     INT         NOT NULL DEFAULT 0,
  deliveries        INT         NOT NULL DEFAULT 0,
  acked_occurrences INT         NOT NULL DEFAULT 0,  -- vocab:allow -- schema history: a shipped migration states the world as it was at its own version and is not editable. ADR 0036 renames this in 00052.
  ...
  CONSTRAINT alert_quality_nonneg_ck CHECK (
      occurrences >= 0 AND notifications >= 0 AND deliveries >= 0 AND acked_occurrences >= 0  -- vocab:allow -- migration history: this file states the schema as it was at its own version, and a shipped migration is not editable. ADR 0036 renamed the entity and 00052 carries the rename.
      AND auto_resolved >= 0 AND expired >= 0 AND total_firing_seconds >= 0 AND flap_transitions >= 0),
  CONSTRAINT alert_quality_acked_ck  CHECK (acked_occurrences <= occurrences),  -- vocab:allow -- migration history: this file states the schema as it was at its own version, and a shipped migration is not editable. ADR 0036 renamed the entity and 00052 carries the rename.
...
COMMENT ON COLUMN alert_quality_daily.occurrences IS 'Episodes that STARTED on this day for this (cluster, alertname).';  -- vocab:allow -- migration history: this file states the schema as it was at its own version, and a shipped migration is not editable. ADR 0036 renamed the entity and 00052 carries the rename.
...
COMMENT ON COLUMN alert_quality_daily.acked_occurrences IS 'Episodes acknowledged by SOMEBODY. Never by whom -- that is the whole point (R8). Capped at occurrences.';  -- vocab:allow -- migration history: this file states the schema as it was at its own version, and a shipped migration is not editable. ADR 0036 renamed the entity and 00052 carries the rename.

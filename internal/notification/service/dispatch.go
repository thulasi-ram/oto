...
type DispatchConfig struct {
	...
	Gates         GateFactory
	Enqueuer      Enqueuer
	// ⛔ `Settings` WAS HERE AND IS DELETED (git-bug bd0fb1d). Dispatch read the
	// org's tuning for exactly ONE thing — the unacked reminder's mention audience
	// (ADR 0020) — and with the reminder withdrawn and the mention gone with it,
	// dispatch has no reason to know an org's settings at all. `notify` still
	// reads them; this seam does not.
	BaseURL      string
	MaxInstances int
